"""
Evaluate the base model with a LoRA adapter on a CSV test set.
"""
...
def parse_args() -> argparse.Namespace:
    # Parse LoRA evaluation CLI arguments.
    p = argparse.ArgumentParser(description="Evaluate base model + LoRA on a test CSV")
    p.add_argument(
        "--model-path",
        type=str,
        default="models/Qwen2.5-1.5B-Instruct",
        help="Base model (same as training)",
    )
    p.add_argument(
        "--adapter-path",
        type=str,
        required=True,
        help="Directory with LoRA weights (e.g. outputs/qwen-sft-lora/final_adapter)",
    )
...
def main() -> None:
    # Run evaluation with adapter-loaded model and report metrics.
